...
library(dplyr)
license.df %>% 
  filter(thing_author_id=="MakerBot")
```
...
license.df <- full_join(license.df, temp, by="thing_author_id") %>% 
...
license.df <- license.df %>% 
               mutate(entrepreneurial.strategy=
                        ifelse(license %in% c("All Rights Reserved",   "Attribution - Non-Commercial - No Derivatives",  "Creative Commons - Attribution - Non-Commercial"),"1",
                        ifelse(license %in% c("BSD License", "Creative Commons - Attribution", "Creative Commons - Attribution - Share Alike"),"2",
