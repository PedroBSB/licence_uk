...
save.image("madeModel1.RData")
```

```{r, echo=FALSE,eval=TRUE,message=FALSE,warning=FALSE, message=FALSE, warning=FALSE}
load("madeModel1.RData")
library(pscl)
library(stargazer)
summary(m1)
```
...
save.image("outModel2.RData")
```

```{r, echo=FALSE,eval=TRUE,message=FALSE,warning=FALSE, message=FALSE, warning=FALSE}
load("outModel2.RData")
library(pscl)
library(stargazer)
summary(m2)
```
...
save.image("madeModel2.RData")

```

```{r, echo=FALSE,eval=TRUE,message=FALSE,warning=FALSE, message=FALSE, warning=FALSE}
load("madeModel2.RData")
library(pscl)
library(stargazer)
summary(m2)
```
...
save.image("outModel3.RData")
```

```{r, echo=FALSE,eval=TRUE,message=FALSE,warning=FALSE, message=FALSE, warning=FALSE}
load("outModel3.RData")
library(pscl)
library(stargazer)
summary(m3)
```
...
m3 <- zeroinfl(made_count ~ appropriability_strategy+ key.event+LC1+LC2+LC3+ files_count+thing_like_count+
                 (LC1*key.event+LC2*key.event+LC3*key.event)+(appropriability_strategy*key.event), data = license.df, dist = "negbin", EM = TRUE)
save.image("madeModel3.RData")

```

```{r, echo=FALSE,eval=TRUE,message=FALSE,warning=FALSE, message=FALSE, warning=FALSE}
load("madeModel3.RData")
library(pscl)
library(stargazer)
summary(m3)
```
